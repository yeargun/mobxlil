...
var ObservableValue = class extends Atom {
  constructor(value, enhancer_, name_ = false ? "ObservableValue@" + getNextId4() : "ObservableValue", notifySpy = true, equals_ = compareDefault) {
    super(name_);
...
var ObservableSet = class {
  constructor(initialData, enhancer = deepEnhancer, name_ = false ? "ObservableSet@" + getNextId5() : "ObservableSet") {
    this.name_ = name_;
...
Extending builtins is not supported.`
  ...
  const name = options?.name ?? (false ? `${isPlainObject(target) ? "ObservableObject" : target.constructor.name}@${getNextId6()}` : "ObservableObject");
...
var ObservableArrayAdministration = class {
  constructor(name = false ? "ObservableArray@" + getNextId8() : "ObservableArray", enhancer, owned_) {
    this.owned_ = owned_;
...
function createObservableArray(initialValues, enhancer, name = false ? "ObservableArray@" + getNextId8() : "ObservableArray", owned = false) {
  return initObservable(() => {
