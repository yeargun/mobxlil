...
var ObservableArrayAdministration = class {
  constructor(name = false ? "ObservableArray@" + getNextId2() : "ObservableArray", enhancer, owned_) {
    this.owned_ = owned_;
  ...
  set_(index, newValue) {
    ...
    if (index < values2.length) {
      ...
      const newItems = Array.from({ length: index + 1 - values2.length });
      newItems[newItems.length - 1] = newValue;
      this.spliceWithArray_(values2.length, 0, newItems);
    }
  }
};
function createObservableArray(initialValues, enhancer, name = false ? "ObservableArray@" + getNextId2() : "ObservableArray", owned = false) {
...
var ObservableValue = class extends Atom {
  constructor(value, enhancer_, name_ = false ? "ObservableValue@" + getNextId3() : "ObservableValue", notifySpy = true, equals_ = compareDefault) {
    super(name_);
...
var ObservableSet = class {
  constructor(initialData, enhancer = deepEnhancer, name_ = false ? "ObservableSet@" + getNextId4() : "ObservableSet") {
    this.name_ = name_;
...
function make_2(adm, key, descriptor, source) {
...
function make_3(adm, key, descriptor) {
  return this.extend_(adm, key, descriptor, false) === null ? 0 /* Cancel */ : 1 /* Break */;
}
...
function assertComputedDescriptor(adm, { annotationType_ }, key, { get: get2 }) {
  if (false) {
    die(
      `Cannot apply '${annotationType_}' to '${adm.name_}.${key.toString()}':
...
function make_4(adm, key, descriptor, source) {
  if (descriptor.get) {
...
var REMOVE = "remove";
var ObservableObjectAdministration5 = class {
...
Extending builtins is not supported.`
  ...
  const name = options?.name ?? (false ? `${isPlainObject(target) ? "ObservableObject" : target.constructor.name}@${getNextId5()}` : "ObservableObject");
  const adm = new ObservableObjectAdministration5(
    target,
    /* @__PURE__ */ new Map(),
    String(name),
...
var isObservableObjectAdministration = createInstanceofPredicate(
  ...
  ObservableObjectAdministration5
);
...
// core/reaction.ts
var Reaction = class {
  constructor(name_ = false ? "Reaction@" + getNextId6() : "Reaction", onInvalidate_, errorHandler_, requiresObservable_) {
    this.name_ = name_;
...
var ObservableMap = class {
  constructor(initialData, enhancer_ = deepEnhancer, name_ = false ? "ObservableMap@" + getNextId7() : "ObservableMap") {
    this.enhancer_ = enhancer_;
    this.name_ = name_;
    initObservable(() => {
...
var ComputedValue = class {
  ...
  constructor(options) {
    ...
    this.name_ = options.name || (false ? "ComputedValue@" + getNextId8() : "ComputedValue");
    if (options.set) {
...
function allowStateChanges(allowStateChanges2, func) {
  const prev = allowStateChangesStart2(allowStateChanges2);
  try {
    return func();
  } finally {
...
function allowStateChangesStart2(allowStateChanges2) {
...
function make_6(adm, key, descriptor, source) {
...
function extend_6(adm, key, descriptor, proxyTrap) {
...
function getAtom(thing, property) {
  if (typeof thing === "object" && thing !== null) {
    ...
    if (isObservableObject(thing)) {
      ...
      const observable2 = adm.values_.get(property) ?? adm.materializeLazyComputed_(property) ?? adm.materializeLazyObservable_(property);
      if (!observable2) {
...
function initObservable(cb) {
  ...
  const allowStateChanges2 = false ? allowStateChangesStart(true) : true;
  startBatch();
  try {
    return cb();
  } finally {
...
function autorun(view, opts = EMPTY_OBJECT) {
  ...
  const name = opts?.name ?? (false ? view.name || "Autorun@" + getNextId9() : "Autorun");
...
function reaction(expression, effect, opts = EMPTY_OBJECT) {
  if (false) {
    if (!isFunction6(expression) || !isFunction6(effect)) {
      die("First and second argument to reaction should be functions");
    }
    if (!isPlainObject3(opts)) {
      die("Third argument of reactions should be an object");
    }
  }
  const name = opts.name ?? (false ? "Reaction@" + getNextId9() : "Reaction");
...
function _when(predicate, effect, opts) {
  ...
  opts.name = false ? opts.name || "When@" + getNextId10() : "When";
...
// api/makeObservable.ts
function makeObservable(target, annotations, options) {
  initObservable(() => {
    const adm = asObservableObject(target, options)[$mobx];
    ownKeys(annotations).forEach((key) => make_7(adm, key, annotations[key]));
  });
  return target;
}
...
function make_7(adm, key, annotation) {
  if (annotation === true) {
