...
var ComputedValue = class {
  ...
  computeValue_(track) {
    ...
    const prev = false ? allowStateChangesStart2(false) : false;
    let res;
    if (track) {
...
var Atom = class {
   ...
   */
  constructor(name_ = false ? "Atom@" + getNextId2() : "Atom") {
    this.name_ = name_;
  }
  flags_ = 0;
  observers_ = /* @__PURE__ */ new Set();
...
function autorun(view, opts = EMPTY_OBJECT) {
  ...
  const name = opts?.name ?? (false ? view.name || "Autorun@" + getNextId4() : "Autorun");
...
function reaction(expression, effect, opts = EMPTY_OBJECT) {
  if (false) {
    if (!isFunction4(expression) || !isFunction4(effect)) {
  ...
  const name = opts.name ?? (false ? "Reaction@" + getNextId4() : "Reaction");
