...
Extending builtins is not supported.`
  ...
  const name = options?.name ?? (false ? `${isPlainObject(target) ? "ObservableObject" : target.constructor.name}@${getNextId4()}` : "ObservableObject");
...
var ObservableArrayAdministration = class {
  constructor(name = false ? "ObservableArray@" + getNextId5() : "ObservableArray", enhancer, owned_) {
    this.owned_ = owned_;
...
function createObservableArray(initialValues, enhancer, name = false ? "ObservableArray@" + getNextId5() : "ObservableArray", owned = false) {
  return initObservable(() => {
...
var ComputedValue = class {
  ...
  constructor(options) {
    ...
    this.name_ = options.name || (false ? "ComputedValue@" + getNextId6() : "ComputedValue");
    if (options.set) {
...
var Atom = class {
   ...
   */
  constructor(name_ = false ? "Atom@" + getNextId7() : "Atom") {
    this.name_ = name_;
  }
  flags_ = 0;
  observers_ = /* @__PURE__ */ new Set();
...
var ObservableMap = class {
  constructor(initialData, enhancer_ = deepEnhancer, name_ = false ? "ObservableMap@" + getNextId8() : "ObservableMap") {
    this.enhancer_ = enhancer_;
    this.name_ = name_;
    initObservable(() => {
