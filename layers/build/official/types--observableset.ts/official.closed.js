...
Extending builtins is not supported.`
  ...
  const name = options?.name ?? (false ? `${isPlainObject(target) ? "ObservableObject" : target.constructor.name}@${getNextId2()}` : "ObservableObject");
...
// core/reaction.ts
var Reaction = class {
  constructor(name_ = false ? "Reaction@" + getNextId3() : "Reaction", onInvalidate_, errorHandler_, requiresObservable_) {
    this.name_ = name_;
...
var ObservableMap = class {
  constructor(initialData, enhancer_ = deepEnhancer, name_ = false ? "ObservableMap@" + getNextId4() : "ObservableMap") {
    this.enhancer_ = enhancer_;
    this.name_ = name_;
    initObservable(() => {
...
var ObservableSet = class {
  constructor(initialData, enhancer = deepEnhancer, name_ = false ? "ObservableSet@" + getNextId8() : "ObservableSet") {
    this.name_ = name_;
