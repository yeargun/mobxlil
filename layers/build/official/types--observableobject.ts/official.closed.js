...
var ObservableArrayAdministration = class {
  constructor(name = false ? "ObservableArray@" + getNextId3() : "ObservableArray", enhancer, owned_) {
    this.owned_ = owned_;
...
function createObservableArray(initialValues, enhancer, name = false ? "ObservableArray@" + getNextId3() : "ObservableArray", owned = false) {
...
var ComputedValue = class {
  ...
  constructor(options) {
    ...
    this.name_ = options.name || (false ? "ComputedValue@" + getNextId4() : "ComputedValue");
    if (options.set) {
...
var Atom = class {
   ...
   */
  constructor(name_ = false ? "Atom@" + getNextId5() : "Atom") {
    this.name_ = name_;
  }
  flags_ = 0;
  observers_ = /* @__PURE__ */ new Set();
...
var ObservableValue = class extends Atom {
  constructor(value, enhancer_, name_ = false ? "ObservableValue@" + getNextId6() : "ObservableValue", notifySpy = true, equals_ = compareDefault) {
    super(name_);
  ...
  has_(key) {
    ...
    if (!entry) {
      entry = new ObservableValue(
        key in this.target_,
        referenceEnhancer,
        false ? `${this.name_}.${stringifyKey2(key)}?` : "ObservableObject.key?",
        false
      );
...
Extending builtins is not supported.`
  ...
  const name = options?.name ?? (false ? `${isPlainObject(target) ? "ObservableObject" : target.constructor.name}@${getNextId8()}` : "ObservableObject");
...
export {
  ObservableObjectAdministration5 as ObservableObjectAdministration,
  asObservableObject,
  assertAnnotable,
  isObservableObject,
